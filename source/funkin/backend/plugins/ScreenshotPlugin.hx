...
class ScreenshotPlugin extends FlxBasic
{
	...
	public static function init():Void FlxG.plugins.addPlugin(new ScreenshotPlugin({}));
	...
	function openScreenshotsFolder():Void FileUtil.openFolder(SCREENSHOT_FOLDER);
	...
	static function makeScreenshotPath():Void FileUtil.createDirIfNotExists(SCREENSHOT_FOLDER);
	...
	function saveBufferedScreenshots(screenshots:Array<Bitmap>, screenshotNames:Array<String>):Void
	{
		...
		if (!ClientPrefs.flashing && !ClientPrefs.previewOnSave) showFancyPreview(screenshots[screenshots.length - 1]); // show the preview for the last screenshot
	}
